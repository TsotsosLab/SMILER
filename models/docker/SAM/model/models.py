...
def upsampling(x):
    return T.nnet.abstract_conv.bilinear_upsampling(
        input=x, ratio=upsampling_factor, num_input_channels=1, batch_size=b_s)
...
def correlation_coefficient(y_true, y_pred):
    ...
    den = K.sqrt((sum_x_square - K.square(sum_x) / N) *
                 (sum_y_square - K.square(sum_y) / N))
...
def nss(y_true, y_pred):
    ...
    y_std = K.repeat_elements(
        K.expand_dims(
            K.repeat_elements(
                K.expand_dims(K.expand_dims(y_std)), shape_r_out, axis=-1)),
        shape_c_out,
        axis=-1)

    y_pred = (y_pred - y_mean) / (y_std + K.epsilon())

    return -(K.sum(K.sum(y_true * y_pred, axis=2), axis=2) / K.sum(
        K.sum(y_true, axis=2), axis=2))
...
def sam_vgg(x):
    ...
    # Learned Prior (1)
    priors1 = LearningPrior(
        nb_gaussian=nb_gaussian, init=gaussian_priors_init)(x[1])

    concateneted = merge([att_convlstm, priors1], mode='concat', concat_axis=1)
    learned_priors1 = AtrousConvolution2D(
        512, 5, 5, border_mode='same', activation='relu',
        atrous_rate=(4, 4))(concateneted)

    # Learned Prior (2)
    priors2 = LearningPrior(
        nb_gaussian=nb_gaussian, init=gaussian_priors_init)(x[1])

    concateneted = merge(
        [learned_priors1, priors2], mode='concat', concat_axis=1)
    ...
    outs = Convolution2D(
        1, 1, 1, border_mode='same', activation='relu')(learned_priors2)
...
def sam_resnet(x):
    ...
    conv_feat = Convolution2D(
        512, 3, 3, border_mode='same', activation='relu')(dcn.output)
