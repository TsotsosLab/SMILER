...
class Salicon:
    def __init__(self,
                 prototxtpath='salicon.prototxt',
                 model='salicon_osie.caffemodel'):
    ...
    def compute_saliency(self, image_path):
        ...
        coarse_img = scipy.ndimage.interpolation.zoom(
            im,
            tuple(COARSE_SCALE / np.asarray(im.shape, dtype=np.float32)),
            np.dtype(np.float32),
            mode='nearest')
        assert (coarse_img.shape == (1, 3, 600, 800))
        fine_img = scipy.ndimage.interpolation.zoom(
            im,
            tuple(FINE_SCALE / np.asarray(im.shape, dtype=np.float32)),
            np.dtype(np.float32),
            mode='nearest')
        ...
        sal_map = scipy.ndimage.interpolation.zoom(
            sal_map,
            tuple(
                np.asarray(im.shape[2:], dtype=np.float32) / np.asarray(
                    sal_map.shape, dtype=np.float32)),
            np.dtype(np.float32),
            mode='nearest')
        return sal_map
