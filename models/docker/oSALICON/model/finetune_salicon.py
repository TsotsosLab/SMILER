...
for i in range(1001, 1701):
    im = np.array(
        Image.open(training_data_path + 'coarse_images/' + str(i) + '.jpg'),
        dtype=np.float32)  # in RGB
...
for i in range(1001, 1701):
    im = np.array(
        Image.open(training_data_path + 'fine_images/' + str(i) + '.jpg'),
        dtype=np.float32)  # in RGB
    # put channel dimension first
    im = np.transpose(im, (2, 0, 1))
    # switch to BGR
...
for i in range(1001, 1701):
    im = np.array(
        Image.open(training_data_path + 'fixation_images/' + str(i) + '.jpg'),
        dtype=np.float32)
    im = im[None, None, :]
...
while time.time() - start_time < 43200:
    ...
    for i in range(0, len(batch)):
        ...
        if int(time.time() - start_time) % 10000 == 0:
            solver.net.save(
                'train_output/finetuned_salicon_{}.caffemodel'.format(
                    idx_counter))
    solver.net.save(
        'train_output/finetuned_salicon_{}.caffemodel'.format(idx_counter))
