function salmap = saliencymap(imfile, options)

salSize   = options.size;
ratio     = options.quantile;
modeltype = options.modeltype;

% image argument should now be passed in directly as an image matrix to
% comport with the SMILER code format.
% - Calden Wloka, July 2015
% % read image
% disp('Reading image.');
% g = double(imread(imfile));
% [height, width, ch] = size(g);
[height, width, ch] = size(imfile);
Img = imfile;

%resize the original image
if height > width
    Img = imresize(imfile, [salSize salSize]);
else
    Img = imresize(imfile, [salSize salSize]);
end
...
% This line removed to allow SMILER to control the colour space over which
% models operate.
...
% These lines modified to reflect arbitrary colour spaces
% - Calden Wloka, December 2018
% F(:,:,1) = L;
% F(:,:,2) = a;
% F(:,:,3) = b;
...
% <!> Code set to only apply when smoothing set to default
% - Calden Wloka, July 2015
if(strcmp(options.do_smoothing, 'default'))
    kSize = max([height, width])*0.02; 
